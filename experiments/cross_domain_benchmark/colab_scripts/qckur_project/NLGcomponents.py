...
class RegressionTemplateBasedTextGeneration():
    ...
    def Q_and_A_about_coefficients(self, coef_pval_df, selected_dependent_var, numeric_standardized=False):
        questions = []
        answers = []

        for index, row in coef_pval_df.iterrows():
            question = linearQuestion.render(
                section=2,
                xcol=row['Xcol'],
                ycol=selected_dependent_var
            )

            interpretation_unit = None
            feature_type = None

            if 'InterpretationUnit' in coef_pval_df.columns:
                interpretation_unit = row.get('InterpretationUnit')

            if 'FeatureType' in coef_pval_df.columns:
                feature_type = row.get('FeatureType')

            if interpretation_unit is None:
